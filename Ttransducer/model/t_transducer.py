...
class T_Transducer(nn.Module):
    ...
    def forward(self, inputs_dict, targets_dict):
        ...
        concat_targets = F.pad(targets, pad=(1, 0, 0, 0), value=1)
    ...
    def rna_decode(self, enc_state, len):
        ...
        for i in range(len):
            ...
            if i == len-1:
                logits = self.jointnet(dec_state.view(-1), enc_state[:,i,:].view(-1))
            else:
                logits = self.jointnet(dec_state.view(-1), enc_state[:, i,:].view(-1))
    ...
    def rnnt_decode(self, enc_state, len):
        ...
        for i in range(len):
            ...
            if i == len-1:
                ...
                logits = self.jointnet(dec_state.view(-1), enc_state[:, i:1+i,:].view(-1))
...
class T_LSTM_Transducer(nn.Module):
    ...
    def rna_decode(self, enc_state, len):
        ...
        for i in range(len):
            dec_state, _ = self.labelencoder(label)
