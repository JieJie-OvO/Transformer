...
class Transducer(nn.Module):
    ...
    def recognize_with_latency(self, inputs_dict, LM=None):
        ...
        if self.predict_strategy == "RNA":
            for i in range(batch_size):
                decoded_seq = self.rna_decode_latency(enc_states[i], inputs_length[i], LM)
                results.append(decoded_seq)
        else:
            for i in range(batch_size):
                decoded_seq = self.rnnt_decode_latency(enc_states[i], inputs_length[i], LM)
                results.append(decoded_seq)
        
        return results
    

    def rna_decode_latency(self, enc_state, lengths, LM=None):
        res1 = []
        zero_token = torch.LongTensor([[0]])
        if enc_state.is_cuda:
            zero_token = zero_token.cuda()
    ...
    def rnnt_decode_latency(self, enc_state, lengths, LM=None):
        res1 = []
        zero_token = torch.LongTensor([[0]])
        if enc_state.is_cuda:
            zero_token = zero_token.cuda()
